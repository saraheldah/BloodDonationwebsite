...
namespace BloodDonationweb.Controllers
{
    public class RegistrationController : MyBaseController
    {
        ...
        {
            if (email is null || password is null || fname is null || lname is null || phone is null || city == 0 || gender == 3 || bloodType == 0 )
                {
        ...
        public IActionResult AddUserAction(string email, string password, string fname, string lname, string phone,
            DateTime birthDate, int city, int gender, int bloodType)
        {
            ...
            if (email is null || password is null || fname is null || lname is null || phone is null || city == 0 || gender == 3 || bloodType == 0 )
            {
                return RedirectToAction("AddUser", "Registration", "0");
            }
                
            email = email.Trim().ToLower();
            password = password.Trim();
            var user = _userManager.GetByEmail(email);
            if (user != null) return RedirectToAction("AddUser", "Registration", "1"); 
            var newUserEntity = _userManager.UserEntity(email, password, fname, lname, phone, birthDate, city, gender, bloodType, Role.User);
            _userManager.Add(newUserEntity);
            return RedirectToAction("AddUser", "Registration", "2");
        }
    }
}
